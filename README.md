...
AI 安全 **80% 是实战**。这一专题独立成篇，两篇递进：**06 篇**讲「LLM 作为内容生成器」的安全（OWASP LLM Top 10）；**07 篇**深入「LLM 变成会动手的 Agent」之后的全新战场（OWASP Agentic Top 10 / ASI）。每条都给 **原理 → 真实案例（带 CVE）→ 精准攻击手法（可复现）→ 防御**，含攻防工具链、合法靶场与成为 AI 安全专家的成长路径。
...
### 🧩 专题：开源 AI 应用项目精选（站在巨人肩膀上学）

学 AI 应用最快的路不是从零硬写，而是**读懂一个成熟项目怎么把"调用大模型"做成"能上线的产品"**。这一专题精选了六大类高星可自部署项目（一站式平台 / 对话前端 / RAG 知识库 / AI 编程 / Agent 框架 / 基础设施），每个都讲清 **它解决什么问题 → 拆源码能学到什么 → 怎么上手**，并给出一套「会用→拆解→复刻→改造」的四步内化法。**不甩链接，直接告诉你每个项目值得学什么。**
